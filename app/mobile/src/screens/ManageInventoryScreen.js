  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    try {
      const response = await fetch(
        `http://stockmanager.alexisprovo.fr/api/reference/get?companyId=${company.id}`
      );
      const referencesData = await response.json();
      const scannedReference = referencesData.find(
        (reference) => reference.barcode_value === data
      );
      console.log(scannedReference)
      if (scannedReference) {
        Alert.alert(
          "Modification de référence",
          `Le code barre ${data} correspond à la référence ${scannedReference.name}, souhaitez-vous modifier la référence ou créer un nouvel article ?`,
          [
            {
              text: "Modifier la référence",
              onPress: () => {
        ...
        console.log(
          `Le code barre ${data} n'a pas été trouvé dans les références`
        );
